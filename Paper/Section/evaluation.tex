...
Monte-Carlo harness; lines: closed form (Eq.~\eqref{eq:psucc}). The floor peaks at
$\psucc=\ArtifactPsuccFloor$; extra equivocators widen the success band.}
\label{fig:psucc}
\end{figure}
...
$\Cop=1$), regenerated by the artifact.}
\label{tab:breakeven}
\small
...
$\psucc=0.85$---the strong split-control/view-control regime; uncontrolled-split floor
$\ArtifactPsuccFloor$). The $E[\Pi]=0$ contour separates the profitable region (both gates hold) from
the rest; the finality-gated baseline is non-positive everywhere.}
\label{fig:phase}
\end{figure}
...
\caption{RQ3. Profit over $(q,\Eval)$ at a window-open latency ratio. The break-even frontier shifts
linearly with the enforcement probability $q$.}
\label{fig:profitqV}
\end{figure}
...
unprofitable. Only window-closing levers (\yes) work. Regenerated by the artifact.}
...
\caption{RQ5. Effect of each defensive lever on $\mathbb{E}[\Pi]$ ($N=31$, $V=60s$). Only levers that
reduce $\pwin$ (finality gating; faster evidence) make the attack unprofitable; economic levers only
dent it.}
\label{fig:defenses}
\end{figure}
...
measured latency ratio. An independent discrete-event protocol simulation with real signatures
reproduces the $\rho\approx1$ threshold.}
\label{fig:testbed}
\end{figure}
...
2026; confirm against current specifications before camera-ready). $N$ is the committee size (the
per-shard committee for sharded rows; the validator/finality-committee size otherwise), distinct from
the measurement sample size $n_{\mathrm{obs}}$ of Table~\ref{tab:measured}. $V^{*}$ is in multiples of one
validator's stake-at-risk. The \emph{Window (G1)} column is the settlement-gate state
(\gated/\openg/\dependsg), and \emph{Verdict} is the resulting one-word outcome: \immune\ when G1 is
closed by construction, \hardgate\ when (G2) is hard to clear because $q\!\approx\!1$, and \exposed\
otherwise. $q$ here is the documented-parameter estimate; \S\ref{sec:measured} measures
the operative evidence-expiry-based value. Cosmos~Hub and Ethereum~PoS are non-sharded accountable-BFT
reference points (IBC finality-gating and single-chain accountability, respectively), included as
comparators; the fast-exit row is a labelled \emph{hypothetical}, not a deployed system.}
